# Architecture Specification — RSNN Modulare per Classificazione Temporale
...
### 1.3. Rumore additivo
- Opzionale, controllato dal flag `add_noise` (booleano).
- Se attivo, durante la **forward pass** viene applicato rumore gaussiano indipendente per neurone.
- Il rumore è **dinamico** (calcolato ad ogni forward), non statico sui pesi iniziali.
- **Nota:** encoder di tipo rate-based (es. Poisson/Hz) **non** ricevono parametri di rumore. Il rumore è gestito a livello di neuroni LIF/ALIF nelle fasi successive.
...
- Parametri di rumore gaussiano sono gestiti a livello di neurono LIF/ALIF durante la forward pass.
...
| ADR-003 | Rumore gaussiano solo su neuroni LIF/ALIF in forward pass, mai sugli encoder | Accettata |
...
*Documento generato in fase SPEC. Prossima fase: SETUP REPO (venv, struttura cartelle, VSCode Remote-SSH).*
